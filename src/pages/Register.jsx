...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    setErrorMessage("");
    try {
      const userData = await postData("/register", data, false);
      ...
      <div className="h-screen mx-auto container flex items-center justify-center">
        <div className="w-full sm:max-w-md mt-6 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg">
          ...
          <div className="mt-5 pb-5">
            ...
            <p className="text-sm mt-4"><span className="text-semibold ">Already have an account?</span> <Link to="/login" className="text-blue">Sign in</Link> </p>
          </div>
        </div>
      </div>
    </div>
