...
import { deleteData, getData } from "../services/api/axios";
...
const MyJobs = () => {
  ...
  const fetchData = async () => {
    try {
      const data = await getData("/current-user");
      // console.log(data);
      setJobs(data.user.jobs);
      // setFilteredJobs(data.user.jobs)
      setIsLoading(false);
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    // console.log(id)
    try {
      const data = await deleteData(`/job/${id}`);
      console.log(data);
      // setJobs(data.user.jobs);
      // setIsLoading(false);
      fetchData();
  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-screen-2xl container mx-auto xl:px-24 px-4 py-10">
           <h1 className="text-center font-semibold p-4 text-xl">My Jobs</h1>
      ...
      <section className="py-2">
            <div className="w-full xl:w-8/12 mb-12 xl:mb-0 px-4 mx-auto mt-5">
              {jobs.length > 0 ? (
                <div className="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 rounded card ">
                ...
                <div className="card relative flex bg-white w-full mb-6 rounded h-40  text-center justify-center items-center">
